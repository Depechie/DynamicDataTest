...
namespace StravaNetStandard.Services
{
    ...
    public class StravaWebClient
    {
        ...
        public Task<string> GetAsync(Uri uri, string accessToken)
        {
            if (uri == null)
                throw new ArgumentException("Parameter uri must not be null. Please commit a valid Uri object.");

            if(!StravaHttpClient.DefaultRequestHeaders.Contains("Authorization"))
                StravaHttpClient.DefaultRequestHeaders.Add("Authorization", $"Bearer {accessToken}");
